...
-- Update auth.users OTP expiry to recommended 10 minutes (600 seconds)
UPDATE auth.config 
SET value = '600'
...
-- Add real-time publication for performance metrics
ALTER PUBLICATION supabase_realtime ADD TABLE engine_performance_metrics;

-- Update engine_outputs table with better indexing
CREATE INDEX IF NOT EXISTS idx_engine_outputs_pillar ON engine_outputs(pillar);
CREATE INDEX IF NOT EXISTS idx_engine_outputs_signal ON engine_outputs(signal);
CREATE INDEX IF NOT EXISTS idx_engine_outputs_confidence ON engine_outputs(confidence DESC);
...
ALTER PUBLICATION supabase_realtime ADD TABLE system_health_status;
ALTER PUBLICATION supabase_realtime ADD TABLE engine_execution_history;
