...
export abstract class BaseEngine implements IEngine {
  ...
  private updateMetrics(success: boolean, executionTime: number, confidence: number): void {
    ...
    this.metrics.averageConfidence = prevAvg + (confidence - prevAvg) / this.metrics.totalExecutions;
  }
  ...
  protected getLastReport(): EngineReport | null {
  ...
  public getStatus(): 'running' | 'idle' | 'error' | 'loading' {
